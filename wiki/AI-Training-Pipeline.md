...
This page explains **how** the Q-Mini-WASM / **qminiwasm-core** training path works and **why** it is shaped this way. **Mission Control / Training WUI** uses the **native** path (Go client → C++ **`TrainingEngineService`** over gRPC); the walkthrough below follows the **legacy Python** `run_training_loop` for clarity unless noted. Canonical split: **[docs/TRAINING_NATIVE_PARITY.md](https://github.com/kennetholsenatm-gif/qminiwasm-core/blob/main/docs/TRAINING_NATIVE_PARITY.md)**.
...
The **legacy Python** entrypoint is **`python -m qminiwasm.engine --config configs/training/<profile>.toml`** (or **`python -m qminiwasm.cli train`**), which loads TOML + env-driven [`EngineConfig`](https://github.com/kennetholsenatm-gif/qminiwasm-core/blob/main/qminiwasm/engine/config.py) and calls [`run_training_loop`](https://github.com/kennetholsenatm-gif/qminiwasm-core/blob/main/qminiwasm/training/loop.py) from [`qminiwasm/engine/train.py`](https://github.com/kennetholsenatm-gif/qminiwasm-core/blob/main/qminiwasm/engine/train.py). **Default WUI training** does not use this subprocess; it uses gRPC and the mapping in [`training-wui/trainingconfig`](https://github.com/kennetholsenatm-gif/qminiwasm-core/tree/main/training-wui/trainingconfig).
...
- **Why strict naming matters:** Serving with **legacy** `uvicorn qminiwasm.engine.serve:app` and **`QMINIWASM_CHECKPOINT`** must use the same **adapter** and **cascade** flags as training or weights will not load as expected (`strict=False` still logs missing keys). Align with **[TRAINING_NATIVE_PARITY.md](https://github.com/kennetholsenatm-gif/qminiwasm-core/blob/main/docs/TRAINING_NATIVE_PARITY.md)** when using Go/`qmw-serve` instead.
