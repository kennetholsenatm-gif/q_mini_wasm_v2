...
| Flatten `engine/`, `qminiwasm/`, etc. into top-level domains | **RFC + P2 + P3** — [LAYOUT_REALIGNMENT_RFC.md](LAYOUT_REALIGNMENT_RFC.md); `qminiwasm.tpem` + `qminiwasm.wasm_host` landed; `enclave` is shim; P4/P5 open |
...
**Last updated:** `qminiwasm.wasm_host` (P3) + `enclave` compatibility shims.
