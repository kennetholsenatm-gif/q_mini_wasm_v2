...
|-----|-------------|---------------|
| **No single greenfield orchestration script** | The [Greenfield Deployment Guide](Greenfield-Deployment.md) describes four manual steps. There is no script that automates the sequence (e.g. validate Packer → run security stack → run data stack → deploy WUI) with basic checks. | Add a script (e.g. `scripts/greenfield-deploy.ps1` or `scripts/greenfield-deploy.sh`) that runs the steps in order with env checks and optional prompts. |
| **WUI backend → Data Stack connection** | When running the WUI backend container (Step 4), the guide does not document the environment variables needed to connect to the data stack (PostgreSQL, RabbitMQ) when those stacks run in Docker. | Document in [Greenfield-Deployment.md](Greenfield-Deployment.md) and/or [containers/wui/README.md](../containers/wui/README.md): e.g. `POSTGRES_HOST`, `POSTGRES_PORT`, `RABBITMQ_HOST`, `RABBITMQ_PORT`, or a single `DATABASE_URL` / `BROKER_URL`. |
| **Air-gap image export/import** | Greenfield mentions air-gap but does not provide a script or checklist to export images (e.g. `docker save`) and load them on the target node. | Add a short section or script in `docs/Greenfield-Deployment.md` or `scripts/` for saving/loading required images and copying compose files + `.env`. |
