...
This index links the repository to the four architecture whitepapers and records implementation status. Canonical **Unified Training Matrix** phase names for TOML and logs: `sft`, `progressive_quant`, `mopd`, `router_cispo` (see `[[training_phases]]` in training TOML and [configs/training/schema.toml](../../configs/training/schema.toml)).
...
| Native gRPC / LibTorch training config | [proto/training_engine.proto](../../proto/training_engine.proto), [training-wui/trainingconfig](../../training-wui/trainingconfig), [cpp/training/src/grpc/training_engine_service.cpp](../../cpp/training/src/grpc/training_engine_service.cpp), [cpp/training/src/training_engine.cpp](../../cpp/training/src/training_engine.cpp), [cpp/training/src/libtorch_ternary_trainer.cpp](../../cpp/training/src/libtorch_ternary_trainer.cpp), [cpp/training/src/bloch_attention.cpp](../../cpp/training/src/bloch_attention.cpp) | Partial: native core is **MLP + optional Bloch** (broadcast token + `pos_embed` sequence, not full Python sequence models; true `[B,T,D]` from data is roadmap—see [TRAINING_NATIVE_PARITY.md](../TRAINING_NATIVE_PARITY.md)). `attention_backend=bloch` enables native Bloch before the ternary stack; **tropical** attention remains Python-only. Unified-matrix RL-only phases (`supervised=false`, `cascade_rl=true`): native toy MDP + `CascadeToyPolicy` with **GRPO** or **CISPO** ([`CascadeGRPO`](../../qminiwasm/rl/cascade_grpo.py) / [`CascadeCISPO`](../../qminiwasm/rl/cascade_cispo.py) parity). Phases with **both** `supervised=true` and `cascade_rl=true`: joint step — MSE on `CoreModule` plus weighted toy GRPO/CISPO (`train_step_joint_supervised_cascade`), one backward per parameter set per tick. |
...
- Training tables: [configs/training/schema.toml](../../configs/training/schema.toml), validated by [qminiwasm/engine/training_schema.py](../../qminiwasm/engine/training_schema.py).
- Native training **model geometry and attention** (mapped by the Training WUI into gRPC): under **`[model]`** use `d_model`, `io_d_model`, `num_ternary_blocks`, `attention_backend`, and (when `attention_backend = "bloch"`) `native_bloch_seq_len` / `native_bloch_num_heads`—see [training-wui/trainingconfig/config.go](../../training-wui/trainingconfig/config.go) and [cpp/training/README.md](../../cpp/training/README.md#libtorch-real-tpem-weights-default-on). Canonical proto: [proto/training_engine.proto](../../proto/training_engine.proto) `TrainingConfig`.
