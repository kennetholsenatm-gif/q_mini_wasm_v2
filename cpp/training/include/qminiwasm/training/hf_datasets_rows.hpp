...
/// If ``text_field_order`` is non-empty, only those JSON object keys are concatenated in order (string values);
/// otherwise legacy: all top-level string values in key order, then ``dump()`` fallback.
