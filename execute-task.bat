@echo off
REM Wiki Changes Commit and Cherry-Pick Script
REM Follows exact requirements for the task

echo === Wiki Changes Commit and Cherry-Pick ===
echo.
...
The wiki generator can now be run to populate the wiki with:
- Research papers (Cognitive Ergonomics, Clifford Entanglement, Framework Synthesis)
...
REM Parse worktree list
set WORKTREE_PATH=
set STASH_CREATED=false

for /f "tokens=1,2 delims= " %%a in (worktree-list.txt) do (
    if "%%a"=="worktree" set CURRENT_WORKTREE=%%b
    if "%%b"=="refs/heads/q_mini_wasm_v2" set WORKTREE_PATH=!CURRENT_WORKTREE!
)
del worktree-list.txt
...
if "%WORKTREE_PATH%"=="" (
    ...
    if %HAS_DIFF% neq 0 goto :stash
    if %HAS_CACHED_DIFF% neq 0 goto :stash
    goto :checkout
    ...
    if not "%CURRENT_BRANCH%"=="q_mini_wasm_v2" (
        echo ERROR: Failed to checkout q_mini_wasm_v2
        exit /b 1
    )
) else (
    ...
    REM Step 4: Check for uncommitted changes
    ...
    if %HAS_DIFF% neq 0 goto :stash2
    if %HAS_CACHED_DIFF% neq 0 goto :stash2
    goto :cherry
...
:cherry
REM Step 5: Cherry-pick the task commit
echo.
...
echo.
echo === Final Report ===
for /f "tokens=*" %%i in ('git rev-parse HEAD') do set FINAL_COMMIT=%%i
...
echo Final commit hash: %FINAL_COMMIT%
echo Final commit message: %FINAL_MSG%
echo Stash used: %STASH_CREATED%
echo Conflicts resolved: No (cherry-pick successful)
...
echo 2. Push to remote: git push origin q_mini_wasm_v2
echo 3. Run wiki generator: python generate_wiki_simple.py