    ...
    def request_certificate(
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(
                    f"Certificate request failed: {response.status_code} - {response.text}"
                )
                return {}
    ...
    def get_certificate(self, serial_number: int) -> Dict:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(
                    f"Certificate retrieval failed: {response.status_code} - {response.text}"
                )
                return {}
    ...
    def revoke_certificate(self, serial_number: int, reason: str = "unspecified") -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(
                    f"Certificate revocation failed: {response.status_code} - {response.text}"
                )
                return False
