"""Conditional Masked Diffusion for Vec2Text-RAG Inversion (Ephemeral State Inversion, ESI)

Implements Conditional Masked Diffusion for exact text reconstruction from embeddings under a
bounded **Maximum State Aperture (MSA)**—zero-degradation memory persistence without KV-style
...
class Vec2TextRAG:
    ...
    def __init__(self, diffusion_model: Optional[Any] = None):
...
def reconstruct_memory(
    ...
    *,
    ...
    Args:
        query_vector: Query vector for memory retrieval
        candidate_vectors: List of candidate vectors from quantum routing
        rag: Optional dedicated :class:`Vec2TextRAG` instance (e.g. with a test diffusion mock).

    Returns:
        Reconstructed text string or None if reconstruction fails
    """
    active = rag if rag is not None else vec2text_rag
    return active.reconstruct_memory(query_vector, candidate_vectors)
...
def esi_beam_search_reconstruct(
    ...
    *,
    ...
    Keeps reconstruction compute-bound (no unbounded key-value-style cache growth in linear memory).
    """
    if beam_width < 1:
        beam_width = 1
    ...
    active = rag if rag is not None else vec2text_rag
    return active.reconstruct_memory(query_vector, flat_candidates)
...
def ephemeral_state_inversion_decode(
    ...
    *,
    ...
    if use_beam and beam_width > 1:
        return esi_beam_search_reconstruct(
            query_vector,
            candidate_vectors,
            beam_width=beam_width,
            rag=rag,
        )
    return reconstruct_memory(query_vector, candidate_vectors, rag=rag)
