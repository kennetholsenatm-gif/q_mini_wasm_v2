...
PyTorch-serialized trainable subtensors are the **training-side** artifact; full **WLES** dumps
serialize guest linear memory (:mod:`qminiwasm.enclave.memory_encode` + native hooks). Both may
coexist. Prefer imports from this module; the thin ``training`` persistence shim re-exports the same API.
"""
