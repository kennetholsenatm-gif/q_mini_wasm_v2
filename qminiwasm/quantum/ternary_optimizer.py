...
class GroverTernaryOptimizer(nn.Module):
    ...
    find ternary weights that minimize the loss. When target_loss_fn is
    omitted, falls back to STE-style binarization (no quantum backend required).
    """
    ...
    def forward(
        ...
        """Return ternary weight configuration.

        If target_loss_fn is provided, runs classical local search to minimize
        target_loss_fn(ternary_weight). Otherwise returns STE-style ternary.
        """
        if target_loss_fn is None:
            fn: Optional[Callable[[torch.Tensor], torch.Tensor]] = None
        elif isinstance(target_loss_fn, nn.Module):
            fn = lambda t: target_loss_fn(t)
        else:
            fn = target_loss_fn
        return ternary_optimizer_classical(
            weight,
            self.num_iterations,
            target_loss_fn=fn,
        )
