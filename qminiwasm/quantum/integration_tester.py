...
class IntegrationTestSuite:
    ...
    def generate_large_scale_test_data(
        ...
        num_database_vectors: int = 10000,
        vector_dim: int = 512
        ...
        """
        logger.info(f"Generating large-scale test data: {num_queries} queries, {num_database_vectors} database vectors, dim={vector_dim}")
    ...
    async def test_end_to_end_encrypted_routing(
        ...
        logger.info(f"End-to-end test completed. Accuracy: {accuracy:.4f}, Encrypted avg time: {avg_encrypted_time:.2f}μs")
        
        return results
    
    def _calculate_routing_accuracy(
        self, 
        regular_results: List[List[int]], 
        encrypted_results: List[List[int]]
    ) -> float:
    ...
    def test_cache_effectiveness(self) -> Dict[str, Any]:
        ...
        speedup = avg_cache_miss_time / avg_cache_hit_time if avg_cache_hit_time > 0 else float('inf')
    ...
    async def test_microsecond_performance(
        self, 
        num_queries: int = 1000
    ) -> Dict[str, Any]:
        ...
        for batch_size in batch_sizes:
            ...
            performance_results[batch_size] = {
                ...
                "p95_latency_us": results["performance_report"]["performance_stats"]["p95_latency_us"],
                "p99_latency_us": results["performance_report"]["performance_stats"]["p99_latency_us"],
            }
        ...
        results = {
            ...
            "best_batch_size": min(performance_results.keys(), key=lambda k: performance_results[k]["avg_time_per_query_us"]),
    ...
    def generate_integration_report(self) -> Dict[str, Any]:
        ...
        if "end_to_end_routing" in self.test_results:
            ...
            report["performance_summary"]["end_to_end"] = {
                ...
                "speedup_factor": end_to_end["regular_routing"]["avg_time_us"] / end_to_end["encrypted_routing"]["avg_time_us"],
            }
        ...
        if "performance_scaling" in self.test_results:
            ...
            report["performance_summary"]["scaling"] = {
                ...
                "best_throughput": max(result["throughput_queries_per_sec"] for result in scaling.values()),
                "scaling_efficiency": "Good" if all(result["avg_time_per_query_us"] < 1000 for result in scaling.values()) else "Needs improvement",
            }
        ...
        if "cache_effectiveness" in self.test_results:
            ...
            report["performance_summary"]["cache"] = {
                ...
                "effectiveness": "Excellent" if cache["cache_speedup"] > 50 else "Good" if cache["cache_speedup"] > 10 else "Needs improvement",
            }
        ...
        if "microsecond_performance" in self.test_results:
            ...
            report["performance_summary"]["microsecond"] = {
                ...
                "avg_latency_us": microsecond["performance_results"][microsecond["best_batch_size"]]["avg_time_per_query_us"],
            }
        ...
        if "end_to_end_routing" in self.test_results:
            ...
            if accuracy < 0.9:
                recommendations.append("Consider improving routing accuracy through better QUBO formulation")
        ...
        if "microsecond_performance" in self.test_results:
            meets_req = self.test_results["microsecond_performance"]["meets_microsecond_requirements"]
            if not meets_req:
    ...
    async def run_full_integration_suite(self) -> Dict[str, Any]:
        ...
        logger.info(f"Success Rate: {report['test_summary']['passed_tests']/report['test_summary']['total_tests']*100:.1f}%")
