...
class NoiseAwareQAOA:
    ...
    def _check_degradation_conditions(self, fidelity_metrics: Dict[str, Any]) -> bool:
        ...
        if avg_fidelity < self.low_fidelity_threshold:
            self.logger.warning(f"QPU average fidelity {avg_fidelity} below threshold {self.low_fidelity_threshold}")
            return True
            
        # Check for high two-qubit gate errors
        high_error_gates = [error for error in fidelity_metrics["two_qubit_gate_errors"] if error > 0.1]
        ...
        if fidelity_metrics["decoherence_time"] < 50e-6:  # Less than 50 microseconds
            self.logger.warning(f"Severe decoherence detected: {fidelity_metrics['decoherence_time']}s")
            return True
...
class FailureTaxonomyManager:
    ...
    def __init__(self, config: HierarchicalConfig):
        ...
        self.qaoa_monitor = NoiseAwareQAOA(config, None)  # Will be set when quantum router is available
        
        # Failure tracking
        self.failure_history: List[FailureEvent] = []
        self.active_failures: Dict[FailureCategory, FailureEvent] = {}
        
    def detect_failure(self, failure_category: FailureCategory, details: Dict[str, Any]) -> FailureEvent:
        ...
        self.logger.error(f"Detected {failure_event.severity} {failure_category.value} failure: {details}")
    ...
    def _identify_affected_components(self, category: FailureCategory) -> List[str]:
        ...
        component_map = {
            ...
            FailureCategory.ALGORITHMIC_NON_DETERMINISM: ["deterministic_verifier", "execution_engine"],
    ...
    def get_failure_statistics(self) -> Dict[str, Any]:
        ...
        for failure in self.failure_history:
            ...
            stats["failure_counts"][category_name] = stats["failure_counts"].get(category_name, 0) + 1
