...
func (w *ShadowAgentWrapper) RunShadowTest(agentType string, input interface{}, pythonOutput interface{}) (*ShadowTestResult, error) {
	...
	if w.Mode >= ShadowModeCompare && pythonOutput != nil {
		...
		if ok {
			...
			result.ParityScore, result.Match, result.Differences = compareOutputs(pythonMap, goOutput)
		}
	}
...
// GetTrend returns parity trend over last N results
func (w *ShadowAgentWrapper) GetTrend(window int) float64 {
	w.mu.Lock()
	defer w.mu.Unlock()
...
func (w *ShadowAgentWrapper) ShouldBlockCanary() bool {
	trend := w.GetTrend(w.MinSuccessWindow)
	return trend < w.ParityThreshold
}
