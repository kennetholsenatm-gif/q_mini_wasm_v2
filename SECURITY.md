...
| **SAST / code scanning** | [pyproject.toml](pyproject.toml) `[tool.bandit]`; [.semgrep.yml](.semgrep.yml); CI runs Bandit on `qminiwasm/` and optional `wui/backend/` when present, optional Semgrep. |
