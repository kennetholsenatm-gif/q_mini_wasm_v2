...
def analyze_commit_message(message):
    ...
    if not message.strip():
        suggestions.append((SUGGESTION_LEVELS["info"], "Commit message is empty. Consider adding a descriptive message."))
        return suggestions
    ...
    if len(lines) > 0:
        ...
        if not type_match:
            suggestions.append((SUGGESTION_LEVELS["info"], f"Consider adding a commit type. Common types: {', '.join(STANDARD_TYPES)}"))
        else:
            commit_type = type_match.group(1)
            if commit_type not in STANDARD_TYPES:
                suggestions.append((SUGGESTION_LEVELS["warning"], f"Type '{commit_type}' not standard. Consider: {', '.join(STANDARD_TYPES)}"))
        ...
        if scope_match:
            ...
            if scope not in project_scopes:
                suggestions.append((SUGGESTION_LEVELS["info"], f"Scope '{scope}' not found in project. Common scopes: {', '.join(project_scopes)}"))
        else:
            suggestions.append((SUGGESTION_LEVELS["info"], "Consider adding a scope in parentheses, e.g., 'feat(ui):'"))

        # Check for issue numbers
        issue_match = re.search(r'#\d+', header)
        if not issue_match:
            suggestions.append((SUGGESTION_LEVELS["info"], "Consider adding issue number, e.g., 'fix(auth): resolve #123'"))
        ...
        if any(keyword in header.lower() for keyword in security_keywords):
            suggestions.append((SUGGESTION_LEVELS["info"], "This appears to be security-related. Consider using 'security:' type and adding CVSS score if applicable"))
    ...
    if config.get("doD_standards", True):
        ...
        if not any(keyword in message.lower() for keyword in classification_keywords):
            suggestions.append((SUGGESTION_LEVELS["info"], "Consider adding classification level for DoD compliance"))
