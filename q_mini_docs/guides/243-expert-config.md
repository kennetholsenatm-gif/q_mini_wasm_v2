...
// For 243 experts, expect < 100μs per routing
...
// Load balancing
config.enable_load_balancing = true;
config.load_balance_alpha = 0.01f;
config.load_balance_interval = 100;
...
// Create router
auto router = CreateUnifiedRouter(config);

// Validate
assert(router->Validate243Config());
assert(router->EstimateMemoryUsage() < 100 * 1024 * 1024);  // < 100 MB
```
...
COPY expert_weights.bin /models/
COPY router_config.json /config/

CMD ["./moe_server", "--config", "/config/router_config.json"]
```
...
new_config.use_hierarchical_selection = true;
new_config.topology = UnifiedMoEConfig::TopologyType::HIERARCHICAL;

// 5. Create new router
auto router = CreateUnifiedRouter(new_config);

// 6. Retrain with existing data (experts will specialize further)
```
...
// 4. Initialize remaining 179 experts randomly
for (size_t i = 64; i < 243; ++i) {
    new_router.InitializeExpertRandom(i);
}
```

## See Also

- [Expert Network Architecture Guide](expert-networks.md)
- [MoE Training Guide](moe-training.md)
- [API Reference: UnifiedMoEConfig](../api/unified_config.md)
